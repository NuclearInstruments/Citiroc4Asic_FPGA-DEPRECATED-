...
architecture rtl of spi_ctrl is
  -- clock generator
  constant SYS_FREQ  : integer :=  100000000;  -- 25MHz
  constant SPI_FREQ  : integer :=   1250000;  -- 6.25MHz
...
begin
  ...
  begin
    case state is
      ...
      when WAIT7 => 
		    case cmd is
            when RDCMD | F_RD => next_state <= WAIT8;
      ...
      when RxDATA =>
        if rx_bit_cnt < x"7" then
          ...
          case cmd is
            ...
            when RDSR | RES => next_state <= WAIT7;
            when others => next_state <= CLR_CMD;
          end case;
        end if;
